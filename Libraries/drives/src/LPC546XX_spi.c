...
void spi_mux(SPIN_enum spi_n, SPI_PIN_enum sck_pin, SPI_PIN_enum mosi_pin, SPI_PIN_enum miso_pin, SPI_PIN_enum cs_pin)
{
    ...
    switch(spi_n)
    {
        ...
        case SPI_1:
        {
            ...
            if      (SPI1_MOSI_B10 == mosi_pin)     iocon_init(B4, ALT2 | DIGITAL | FILTEROFF);
            else                                    ASSERT(0);//引脚错误 进入断言失败
            ...
            if      (SPI1_CS0_A13 == cs_pin)        iocon_init(A13,ALT1 | DIGITAL | FILTEROFF | GPIOMODE | I2CFILTEROFF);
            else if (SPI1_CS1_A14 == cs_pin)        iocon_init(A14,ALT1 | DIGITAL | FILTEROFF | GPIOMODE | I2CFILTEROFF);
            else if (SPI_CS_NOT_USE == cs_pin)      ;
            else                                    ASSERT(0);//引脚错误 进入断言失败
        }break;
